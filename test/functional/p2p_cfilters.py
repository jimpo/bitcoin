...
from test_framework.messages import (
    FILTER_TYPE_BASIC,
    NODE_NETWORK, NODE_BLOOM, NODE_COMPACT_FILTERS, NODE_WITNESS, NODE_NETWORK_LIMITED,
...
class CompactFiltersTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        wait_until(lambda: self.nodes[0].getblockcount() == 1000, timeout=5)
        stale_block_hash = self.nodes[0].getblockhash(1000)

        self.nodes[1].generate(1001)
        wait_until(lambda: self.nodes[1].getblockcount() == 2000, timeout=5)
        ...
        default_services = NODE_NETWORK | NODE_BLOOM | NODE_WITNESS | NODE_NETWORK_LIMITED
        ...
        genesis_cfheader = self.nodes[0].getblockfilter(genesis_hash, 'basic')['header']
        ...
        assert_equal(
            response.headers,
            [int(header, 16) for header in (genesis_cfheader, main_cfcheckpt, tip_cfcheckpt)]
        )
        ...
        assert_equal(
            response.headers,
            [int(header, 16) for header in (genesis_cfheader, stale_cfcheckpt)]
        )
        ...
        for request in requests:
            ...
            node1.wait_for_disconnect(timeout=5)
        ...
        for request in requests:
            node0 = self.nodes[0].add_p2p_connection(CFiltersClient())
            node0.send_message(request)
            node0.wait_for_disconnect(timeout=5)
