                                      ...
                                      BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        // REVIEW: Should we disconnect? Mark misbehaving? Simply ignore the request?
        pfrom->fDisconnect = true;
        LogPrintf("Derp 0 %d %d\n", filter_type == BlockFilterType::BASIC, (pfrom->GetLocalServices() & NODE_COMPACT_FILTERS));
        return false;
    }
                               ...
                               CConnman* connman)
{
    ...
    if (!QueryFilterIndexWithRetry(filter_index, index_in_sync,
...
static bool UpdateCFHeadersCache(const BlockFilterIndex& filter_index)
{
    ...
    for (int i = active_chain_cf_headers.size() - 1; i >= 0; i--) {
        if (active_chain.Contains(active_chain_cf_headers[i].first)) {
            active_chain_cf_headers.resize(i + 1);
            break;
        }
    }

    // Populate the checkpoint cache with headers for blocks on the active chain.
    for (uint32_t height = active_chain_cf_headers.size() * CFCHECKPT_INTERVAL;
                                ...
                                CConnman* connman)
{
    ...
    if (!UpdateCFHeadersCache(*filter_index)) {
        return false;
    }

    std::vector<uint256> headers(1 + stop_index->nHeight / CFCHECKPT_INTERVAL);

    // Populate headers.
    int i = headers.size() - 1;
    const CBlockIndex* block_index = stop_index;
    for (; i >= 0; i--) {
        uint32_t height = i * CFCHECKPT_INTERVAL;
